...
export default function Editfooditem({ onClose, data, onChange, onSave }) {
  const { data: dropDownOptions, isLoading } = useGetCategoryQuery();
  console.log("data category check", data?.categoryid);
  ...
  const selectedCategory = (categoryid) => {
    const selectedCategory = dropDownOptions?.data.find(
      ...
      <div className="bg-white rounded-lg shadow-lg w-120 p-6">
        ...
        <div className="mb-4">
          ...
          <Dropdown
            ...
            selected={selectedCategory(data?.categoryid)}
            onChange={(value) =>
