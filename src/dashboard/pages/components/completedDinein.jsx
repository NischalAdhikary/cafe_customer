...
import { Clock, User, MapPin, Loader, Check } from "lucide-react";
import StatusModal from "./statusModal";

const COMPLETED_DINEIN = {
...
export function Completeddinein({ orderdata, loading }) {
  ...
  const dineinOrders =
    orderdata?.filter((order) => order.ordertype === "DINEIN") || [];
    ...
    <div className="w-full h-auto">
      <div className="flex flex-wrap gap-4">
        {dineinOrders.map((item) => (
          ...
          >
            <div
              className={`p-4 rounded-t-lg border-b ${COMPLETED_DINEIN.headerBg} ${COMPLETED_DINEIN.borderColor}`}
            >
              <div className="flex items-center justify-between mb-2">
                <span
                  className={`${COMPLETED_DINEIN.badgeBg} ${COMPLETED_DINEIN.badgeText} text-xs font-medium  px-2 py-1 rounded-full`}
                >
                  {item.ordertype}
                </span>
            ...
            <div className="p-4">
              ...
              <button
                ...
                className={`w-full ${COMPLETED_DINEIN.buttonBg} ${COMPLETED_DINEIN.buttonText} py-2 px-4 rounded-md text-sm font-medium transition-colors duration-200`}
              >
                Pay
              </button>
            </div>
